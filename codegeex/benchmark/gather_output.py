...
def gather_output(
    ...
    if output_prefix is None:
        output_list = glob.glob(f"{output_dir}/*")
    else:
        output_list = glob.glob(os.path.join(output_dir, f"{output_prefix}*"))

    for output_file in output_list:
        if "rank0" in output_file:
            output_prefix_ = output_file.split("_rank0.jsonl")[0]
            rank_files = glob.glob(f"{output_prefix_}_rank*")
            with open(f"{output_prefix_}.jsonl", "w") as f_out:
    ...
    for output_file in output_list:
        ...
        files = [output_file, f"{output_prefix_}_unfinished.jsonl"]
        with open(f"{output_prefix_}_all.jsonl", "w") as f_out:
            for f in files:
                with open(f, "r") as f_in:
                    for line in f_in:
                        f_out.write(line)

        print(f"Gathering finished. Saved in {output_prefix_}_all.jsonl")
